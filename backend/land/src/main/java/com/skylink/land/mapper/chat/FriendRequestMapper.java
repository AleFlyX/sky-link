...
public interface FriendRequestMapper extends BaseMapper<FriendRequest> {
    ...
    @Select("""
        ...
        LIMIT #{offset}, #{size}
        """)
    @Results(id = "incomingFriendRequestRowMap", value = {
    ...
    @Select("""
        ...
        FROM friend_request fr
        JOIN user u
          ON u.user_id = fr.receiver_id
         AND u.is_deleted = 0
        WHERE fr.requester_id = #{currentUserId}
        """)
    long countOutgoingRequests(@Param("currentUserId") Long currentUserId);

    @Select("""
        SELECT
          fr.request_id,
          fr.receiver_id AS target_user_id,
          fr.message,
          fr.status AS request_status,
          u.username,
          u.nickname,
        ...
        LIMIT #{offset}, #{size}
        """)
    @Results(id = "outgoingFriendRequestRowMap", value = {
        @Result(property = "requestId", column = "request_id"),
    ...
    List<SentFriendRequestRow> selectOutgoingRequests(
        @Param("currentUserId") Long currentUserId,
        @Param("offset") long offset,
        @Param("size") int size
    );
