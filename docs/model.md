...
- `department_id` 用于关联 `department.department_id`，服务层会在写入时校验部门存在；当前可重复启动的初始化脚本不为它追加运行时外键。
...
- `leader_id` 用于关联 `user.user_id`，服务层会在写入时校验负责人存在；当前可重复启动的初始化脚本不为它追加运行时外键。
- 部门与用户是一对多关系。
...
- 任务执行人必须与任务创建人属于同一部门，服务层在创建和改派时执行该约束。
...
3. **外键约束**：大多数核心引用关系使用外键保证数据有效性；`user.department_id` 和 `department.leader_id` 这组循环引用由服务层校验，以保持初始化脚本可重复执行且更稳定。
