...
```json
{
  ...
  "content": "Hello",       // 文本内容或文件URL等
  "fileId": null            // 若有文件，关联文件ID
}
```
...
### 7.5 标记消息已读
- **接口**：`PUT /messages/read`
- **描述**：标记某会话的所有消息为已读
- **请求体**：
```json
{
  "receiverId": 1002,   // 单聊对方
  "groupId": 123        // 或群聊ID，二选一
}
```
- **响应**：返回已读消息数量。
...
### 8.1 上传文件
- **接口**：`POST /files`
- **描述**：上传文件，支持 multipart/form-data
- **请求体**：表单数据
  - `file`：文件（二进制）
  - `type`：文件分类（可选，如 image, document, other）
- **响应**：返回文件信息：
```json
{
  "code": 200,
  ...
  "data": {
    "fileId": 2001,
    "fileName": "report.pdf",
    "filePath": "/uploads/files/2026/07/report.pdf",
    "fileSize": 2048000,
    "fileType": "pdf",
    "uploadTime": "2026-07-13T16:00:00Z"
  }
}
```
...
### 8.2 获取文件列表
- **接口**：`GET /files`
- **描述**：获取当前用户上传的文件列表，可筛选
- **Query 参数**：分页、`fileType`（按类型筛选）
- **响应**：分页文件信息列表。
...
### 8.6 分享文件
- **接口**：`POST /files/{fileId}/share`
- **描述**：创建文件分享链接或指定分享给用户/群组
- **请求体**：
```json
{
  "targetUserIds": [1002, 1003],   // 分享给指定用户，可选
  "targetGroupId": 123,            // 或分享给整个群组
  "permission": "view",            // view, edit
  "expireTime": "2026-07-20T16:00:00Z"  // 可选，过期时间
}
```
- **响应**：返回共享记录ID及分享链接（若有）。
...
### 8.8 取消文件分享
- **接口**：`DELETE /files/shares/{shareId}`
- **描述**：删除分享记录
- **响应**：成功消息。
...
### 9.6 设置文档权限
- **接口**：`POST /documents/{documentId}/permissions`
- **描述**：给用户或群组分配文档权限
- **请求体**：
```json
{
...
### 9.8 移除文档权限
- **接口**：`DELETE /documents/{documentId}/permissions/{permissionId}`
- **描述**：删除特定权限记录
- **响应**：成功消息。
...
```json
{
  ...
  "attachments": [2001, 2002]  // 附件文件ID列表
}
```
- **响应**：返回任务对象。
...
```json
{
  ...
  "remindTime": "2026-07-14T08:30:00Z", // 提醒时间，可选
...
### 11.2 获取日程列表
- **接口**：`GET /schedules`
- **描述**：获取当前用户的日程，支持时间范围筛选
- **Query 参数**：
  - `startDate`：起始日期（ISO日期）
  - `endDate`：结束日期
  - `page`, `size`
- **响应**：分页日程列表。

### 11.3 获取日程详情
- **接口**：`GET /schedules/{scheduleId}`
- **响应**：日程详细信息。

### 11.4 更新日程
- **接口**：`PUT /schedules/{scheduleId}`
- **请求体**：字段同创建，可选。
- **响应**：更新后的日程。

### 11.5 删除日程
- **接口**：`DELETE /schedules/{scheduleId}`
- **响应**：成功消息。
...
{
  "title": "关于系统升级的通知",
  "content": "系统将于7月15日进行升级...",
  "targetType": "all",   // all, department, role
  "targetIds": []       // 若指定目标，填写ID列表
}
```
- **响应**：返回公告对象。
...
### 12.2 获取公告列表
- **接口**：`GET /notices`
- **描述**：获取当前用户可查看的公告，按发布时间倒序
- **Query 参数**：分页
- **响应**：分页公告列表。

### 12.3 获取公告详情
- **接口**：`GET /notices/{noticeId}`
- **响应**：公告完整内容。

### 12.4 删除公告（管理员）
- **接口**：`DELETE /notices/{noticeId}`
- **响应**：成功消息。

### 12.5 标记公告已读（可选）
- **接口**：`POST /notices/{noticeId}/read`
- **描述**：记录当前用户已读该公告
- **响应**：成功消息（若需记录已读状态）。

### 12.6 获取未读公告数量
- **接口**：`GET /notices/unread/count`
- **描述**：获取当前用户未读公告数
- **响应**：
```json
{
...
- **接口**：`GET /logs/login`
- **描述**：分页查询登录日志，支持筛选
- **Query 参数**：`userId`、`loginTimeStart`、`loginTimeEnd`、`loginResult`
- **响应**：分页日志记录。

#### 13.4.2 查询操作日志
- **接口**：`GET /logs/operation`
- **描述**：分页查询操作日志
- **Query 参数**：`userId`、`module`、`operation`、`operationTimeStart/End`
- **响应**：分页日志记录。
...
```json
{
  ...
  "data": {
    "totalUsers": 150,
    "onlineUsers": 30,      // 近期在线估算
    "totalGroups": 20,
    "totalMessages": 5000,
    "totalFiles": 300,
    "taskCompletionRate": 0.65,
    "departmentStats": [
      { "departmentId": 1, "departmentName": "研发部", "memberCount": 50 },
      ...
    ],
    "recentMessages": [     // 近7天消息数量
      { "date": "2026-07-07", "count": 120 },
      ...
    ]
  }
}
...
| 消息类型 (messageType) | text, image, file, system                               |
| 文件共享权限 (permission) | view, edit                                           |
| 任务状态 (status)    | 未开始, 进行中, 已完成                                    |
| 任务优先级 (priority) | 1-低，2-中，3-高                                           |
| 日程重复类型 (repeatType) | none, daily, weekly, monthly                          |
