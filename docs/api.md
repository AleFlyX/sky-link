...
```
Access Token 通过登录接口获取，有效期由服务端 `skylink.jwt.ttl` 配置控制。Refresh Token 仅通过 HttpOnly Cookie 保存和传输，不出现在 JSON 响应体或请求体中。
...
### 2.2 用户登录
- **接口**：`POST /auth/login`
- **描述**：支持用户名或邮箱登录，返回 Access Token，并通过 `Set-Cookie` 写入 HttpOnly Refresh Token。
- **请求体**：
```json
{
...
- **描述**：使用 HttpOnly Cookie 中的 Refresh Token 换取新的 Access Token，并轮换 Refresh Token Cookie。
- **请求体**：无。浏览器需携带服务端登录或刷新时写入的 Refresh Token Cookie。
- **响应**：同登录返回的 token 结构；响应体不包含 `refreshToken`。

### 2.4 退出登录
- **接口**：`POST /auth/logout`
- **描述**：清除 Refresh Token Cookie；前端同时清除本地 Access Token。
- **请求头**：不要求携带 Token。
- **响应**：
```json
{
...
### 5.6 获取我发出的好友申请列表
- **接口**：`GET /friends/requests/sent`
- **描述**：获取当前用户主动发出的好友申请，便于查看申请状态
- **Query 参数**：分页
- **响应**：分页申请列表，包含目标用户信息、附言、申请状态、申请时间。
...
### 9.1 创建任务
- **接口**：`POST /tasks`
- **描述**：管理员或具备任务创建权限的角色创建任务；执行人必须是创建者所在部门的有效成员
- **请求体**：
```json
{
  "title": "开发登录模块",
  "content": "实现JWT认证和注册接口",
  "executorId": 1002,        // 同部门负责人ID
  "priority": 1,             // 1低，2中，3高
...
### 9.4 更新任务
- **接口**：`PUT /tasks/{taskId}`
- **描述**：修改任务信息（管理员或创建者）；重新指派执行人时仍只能选择任务创建者所在部门成员
- **请求体**：字段同创建，可选。
- **响应**：更新后的任务。
