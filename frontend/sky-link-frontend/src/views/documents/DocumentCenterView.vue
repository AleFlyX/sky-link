...
const router = useRouter(); const userStore = useUserStore(); const keyword = ref(''); const page = ref(1); const pageSize = 6
...
const statusDialog = ref(false); const statusTarget = ref(null); const statusSaving = ref(false); const statusForm = reactive({ status: 'private' })
...
function hasAdminRole() {
  return (Array.isArray(userStore.user?.roles) ? userStore.user.roles : []).some((role) => {
    ...
    const code = String(role.roleCode || role.code || role.name || role.roleName || role.label || role).trim().toUpperCase()
    return adminRoleCodes.has(code)
  })
}
function canManageRow(row) {
  return row?.permission === 'manage' || Number(row?.creatorId) === Number(userStore.user?.id) || hasAdminRole()
}
...
async function saveStatus() {
  ...
  try {
    ...
    statusDialog.value = false
    ElMessage[result.degraded ? 'warning' : 'success'](result.degraded ? '文档接口不可用，已保存到演示数据' : '可见范围已保存')
    await loadData()
  } catch (error) {
  ...
  <div class="page-shell">
    <AppCard title="在线文档" subtitle="查看文档列表并管理协作者">
      ...
      <AppDataTable :columns="columns" :rows="rows" :loading="loading" :error="loadError" empty-text="暂无文档" @retry="loadData">
        ...
        <template #status="{ value }"><AppStatusTag :label="statusLabels[value] || '仅自己可见'" :tone="value === 'archived' ? 'info' : 'primary'" /></template>
        <template #actions="{ row }">
          <div class="row-actions">
            <AppButton v-permission="'document:update'" variant="primary" @click="openDocument(row)">进入</AppButton>
            <AppButton v-if="canManageRow(row)" v-permission="'document:permission:list'" variant="secondary" @click="openPermissions(row)">协作者</AppButton>
            <AppButton v-if="canManageRow(row)" v-permission="'document:update'" variant="secondary" @click="openStatusDialog(row)">状态管理</AppButton>
          </div>
        </template>
      </AppDataTable>
...
<style scoped>
.document-title{padding:0;border:0;background:transparent;color:var(--color-primary);font:inherit;font-weight:600;cursor:pointer}.row-actions,.grant-form{display:flex;align-items:center;gap:.75rem;flex-wrap:wrap}.grant-form{margin-bottom:1rem}.grant-form .app-input{flex:1}.grant-form .el-select{width:130px}.status-form{display:grid;gap:1rem}.status-form__hint{margin:0;color:var(--color-text-muted);line-height:1.7}.status-form__select{width:100%}.status-form__footer{display:flex;justify-content:flex-end;gap:.75rem}@media(max-width:640px){.grant-form{align-items:stretch;flex-direction:column}.grant-form .el-select{width:100%}}
</style>
