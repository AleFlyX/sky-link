...
vi.mock('../api/document', () => ({
  createCollaborationTicket: vi.fn(async () => ({
    data: { token: 'ticket', websocketUrl: 'ws://127.0.0.1:8180', permission: 'edit' },
  })),
}))
...
describe('collaborative editor connection states', () => {
  ...
  it('moves from connecting to synced and persisted', async () => {
    ...
    const session = scope.run(() => useCollaborationSession(9))
    ...
    provider.emit('stateless', {
      payload: JSON.stringify({ type: 'saved', at: '2026-07-14T12:00:00Z' }),
    })
    expect(session.status.value).toBe('saved')
    expect(session.savedAt.value).toBe('2026-07-14T12:00:00Z')
    scope.stop()
  })
  ...
  it('switches to read-only when authentication is revoked', async () => {
    ...
    const session = scope.run(() => useCollaborationSession(9))
    await session.connect()
    providers[0].emit('authenticationFailed', { reason: 'permission revoked' })
    expect(session.status.value).toBe('readonly')
    expect(session.editable.value).toBe(false)
    expect(session.error.value).toBe('permission revoked')
    scope.stop()
  })
})
